 ...
 */
public class Jigsaw {
	JigsawNode beginJNode;		// 起始状态节点
	JigsawNode endJNode;		// 目标状态节点
	JigsawNode currentJNode;	// 当前状态节点
	private Vector<JigsawNode> openList;	// open表 ：用以保存已发现但未访问的节点
	private Vector<JigsawNode> closeList;	// close表：用以保存已访问的节点
	private Vector<JigsawNode> solutionPath;// 路径  ：用以保存从起始状态到达目标状态的移动路径中的每一个状态节点
